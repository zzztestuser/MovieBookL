...
public class EventsInviteServlet extends HttpServlet {
	...
	protected void doDelete(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int eventID;
		if (StringUtils.isEmpty(request.getParameter("event"))) {
			log.error("No event parameter passed when rejecting invite");
			response.setContentType("text/plain");
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
			response.getWriter().write("No event parameter passed when rejecting invite");
			return;
		} else {
			try {
				eventID = Integer.parseInt(StringUtils.trim(request.getParameter("event")));
		...
		try {
			EventsManager.updateAttendeeInviteStatus(eventID, userID, InviteStatus.REJECTED.code());
			response.setContentType("text/plain");
			response.setCharacterEncoding("UTF-8");
			response.setStatus(HttpServletResponse.SC_NO_CONTENT);
			return;
